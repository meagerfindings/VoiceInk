...
struct VoiceInkApp: App {
    ...
    private static func createPersistentContainer(schema: Schema, logger: Logger) -> ModelContainer? {
        do {
            ...
            // Dictionary configuration — CloudKit disabled (fork uses different team, upstream container won't authenticate)
...
class UpdaterViewModel: ObservableObject {
    ...
    init() {
        ...
        // Personal fork: re-signed to a different team, so upstream-signed
        // automatic updates would clobber the local build. Force auto-checks
        // off regardless of the Sparkle/plist default. Manual checks still work.
